...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'description', 'slug', 
            'inventory', 'unit_price', 'price_with_tax', 
            'collection', 'images']
...
class SimpleProdutSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = ['id','title','unit_price']

class CartItemSerializer(serializers.ModelSerializer):
    product = SimpleProdutSerializer()
...
class CartSerializer(serializers.ModelSerializer):
    ...
    def get_total_price(self,cart):
        return sum([item.quantity * item.product.unit_price for item in cart.item.all()].all())
    class Meta:
        model = Cart
        fields = ['id','items']
...
class AddCartItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self,value):
        if not Product.objects.filter(pk=value).exists():
            raise serializers.ValidationError('No product with the primary key')
        return value

    def save(self, **kwargs):
        cart_id = self.context['cart_id']
        product_id = self.validated_data['product_id']
        quantity = self.validated_data['quantuty']
